...
export function parseSpinResult(txResponse: TransactionResponse): SpinResult | null {
  try {
    // Find the wasm event with spin data
    const wasmEvent = txResponse.logs[0]?.events.find(
      (event) => event.type === 'wasm' && 
      event.attributes.some(attr => attr.key === 'action' && attr.value === 'spin')
    );
    ...
    const traitTarget = (getAttr('trait_target')?.toLowerCase() || 'cap') as 'cap' | 'stem' | 'spores';
